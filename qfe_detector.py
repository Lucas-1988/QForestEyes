...
def install_and_authenticate():
    """Verifica la API de GEE usando la ruta absoluta de Python de QGIS."""
    try:
        import ee
        return True
    except ImportError:
        try:
            ...
            from qgis.utils import iface
            import subprocess
            import sys # Necesario para obtener sys.executable
            
            iface.messageBar().pushMessage("Instalación", "Configurando earthengine-api en el entorno de QGIS...", level=Qgis.Info)
            
            # Usamos sys.executable pero con argumentos que bloquean la apertura de ventanas
            # y forzamos la instalación en el sitio de paquetes del usuario para evitar errores de permisos.
            subprocess.check_call([
                sys.executable, "-m", "pip", "install", "earthengine-api", "--user"
            ], creationflags=0x08000000) # Este flag es CLAVE para no abrir otro QGIS o consolas
            
            # Forzamos a Python a volver a buscar librerías en la carpeta de usuario
            import site
            import importlib
            ...
            iface.messageBar().pushMessage(
                "Error", f"No se pudo instalar automáticamente: {e}.",
